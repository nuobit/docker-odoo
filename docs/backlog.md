...
- [ ] Harden `snapshot.sh` and `db.sh` against path traversal and unsafe `rm -rf`
  ...
  - **Affected commands:** `create` (creates dirs + writes outside SNAPSHOT_DIR), `restore` (reads from arbitrary path), `remove` (deletes arbitrary path). The `rm -rf` in `remove` and the create ERR trap are the most dangerous.
...
- [ ] Extract embedded Python from `snapshot.sh` into standalone scripts
  ...
  - Keeps shell scripts focused on orchestration, Python focused on data processing.
