...
usage() {
  ...
  echo ""
  echo "Configuration (from ${ODOO_CONF} and settings.env):"
  echo "  DB_HOST             PostgreSQL host (or db_host in odoo.conf)"
  echo "  DB_PGUSER           PostgreSQL admin user (default: postgres)"
  echo "  DB_PGDB             PostgreSQL maintenance database (default: postgres)"
  echo "  DB_PGUSER_PASSWORD  PostgreSQL admin password (if set, avoids prompt)"
  echo "  DB_FORCE            Same as -f flag (skip name confirmation)"
  echo "  db_user             PostgreSQL database owner (from odoo.conf)"
  echo "  db_password         PostgreSQL database owner password (from odoo.conf)"
  exit 2
}
