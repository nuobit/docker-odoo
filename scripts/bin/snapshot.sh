...
usage() {
  ...
  echo ""
  echo "Commands:"
  echo "  create [-v|--verbose] [-j|--jobs N] [-n|--note \"text\"] [--no-filestore] <dbname> <snapshot-name>"
  echo "                             Create a snapshot of database and filestore"
  echo "  restore [-f|--force] [-v|--verbose] [-j|--jobs N] <snapshot-name> [dbname]"
  echo "                             Restore a named snapshot (default: original database)"
  echo "  list                       List available snapshots"
  echo "  remove [-f|--force] <snapshot-name>"
  echo "                             Delete a snapshot"
  echo ""
  echo "Options (after subcommand):"
  echo "  -f, --force       Skip confirmation prompts (applies to: restore, remove)"
  echo "  -v, --verbose     Show detailed output from pg_dump, pg_restore, and rsync (applies to: create, restore)"
  # shellcheck disable=SC2154  # SNAPSHOT_JOBS from defaults.env
  echo "  -j, --jobs N      Parallel workers for pg_dump/pg_restore (default: ${SNAPSHOT_JOBS}) (applies to: create, restore)"
  echo "  -n, --note TEXT   Optional description stored in metadata (applies to: create)"
  echo "  --no-filestore    Skip filestore, database only (applies to: create)"
  echo ""
  echo "Configuration:"
